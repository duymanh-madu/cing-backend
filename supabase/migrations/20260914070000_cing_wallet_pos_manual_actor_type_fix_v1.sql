/*
 * CING WALLET POS MANUAL ACTOR TYPE FIX V1
 *
 * Fix only the audit actor category emitted by manual
 * Counter session creation.
 *
 * Authenticated admin identity remains actor_id.
 * Operational actor_type is canonical "cashier".
 */
...
begin
  ...
  then
    raise exception
      'CING_WALLET_POS_MANUAL_ACTOR_CLIENT_EXECUTE_FORBIDDEN';
  end if;
  ...
  then
    raise exception
      'CING_WALLET_POS_MANUAL_ACTOR_SERVICE_ROLE_MISSING';
  end if;
end;
$proof$;
